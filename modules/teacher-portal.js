...
// True only for admin's own internal Registration tab (not teacher mode at all).
// "Preview as Teacher" is meant to faithfully mirror what a real teacher sees, so this
// stays hidden there too, not just for an actual teacher session.
function _regIsAdminPreview(){return !IS_TEACHER_MODE;}
...
async function scheduleEmailPdf(){
  ...
  try{
    ...
    const res=await fetch('/.netlify/functions/send-email',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({to:bk.leaderEmail,subject:`Your Retreat Schedule — ${bk.retreatName||bk.leaderName||''}`,html,cc:_ccList(bk),attachments:[{filename:_scheduleFilename(bk),content:base64,contentType:'application/pdf'}]})});
...
async function sendDepositEmail(){
  ...
  try{
    await _sendEmail(bk.leaderEmail.split(',').map(e=>e.trim()).filter(Boolean),`Deposit Instructions — ${retreat} · ${fmtRange}`,depositHtml,'retreats@amansala.com',_ccList(bk));
...
// ── Send Final Balance — same wire/Zelle/Venmo instructions as the deposit
// email, but for the remaining balance due. Stashes the sent timestamp in the
// packageCustomPrices.__cfg__ config-bag (same pattern used for taxRate, colorIdx,
// etc.) instead of a new DB column, so no schema migration is needed — the
// Dashboard's Overdue Balances row reads this same field to show a "sent" badge.
// Shows a live preview (exactly what the leader will receive) before sending —
// _fbConfirmSend() does the actual send once confirmed in the modal.
let _fbPending=null; // {bkId, to, subject, html, balance}
async function sendFinalBalanceEmail(){
  if(!regSelBk)return;
  const bk=regSelBk;
  if(!bk.leaderEmail){showToast('No email on file for this retreat leader');return;}
  const {balance}=calcBkBalance(bk);
  if(balance<=0){showToast('No balance due — nothing to send');return;}
  const dueDate=(()=>{const d=new Date();d.setDate(d.getDate()+7);return d.toLocaleDateString('en-US',{month:'long',day:'numeric',year:'numeric'});})();
  const ref=`${bk.leaderName||'Retreat'} - ${bk.startDate?fmtDate(bk.startDate):''}`;
  const fmtRange=`${fmtDate(bk.startDate)} – ${fmtDate(bk.endDate)}`;
  const retreat=bk.retreatName||bk.leaderName||'Retreat';
  const finalHtml=`<!DOCTYPE html><html><body style="margin:0;padding:0;background:#f8fafc;font-family:Arial,Helvetica,sans-serif"><table width="100%" cellpadding="0" cellspacing="0"><tr><td><table style="max-width:580px;margin:20px auto;border-radius:12px;overflow:hidden;box-shadow:0 4px 20px rgba(0,0,0,.08)" cellpadding="0" cellspacing="0" width="100%"><tr><td style="background:#1a2332;padding:22px 32px"><span style="color:#4db6ac;font-size:22px;font-weight:700;letter-spacing:3px">AMANSALA</span></td></tr><tr><td style="background:#ffffff;padding:32px;color:#374151;font-size:14px;line-height:1.7"><p>Hi ${bk.leaderName},</p><p>Your retreat is coming up! To finish confirming your reservation, please submit your <strong>remaining balance of ${fmt$(balance)} by ${dueDate}</strong>.</p><table style="width:100%;border-collapse:collapse;margin:20px 0;font-size:13px;border:1px solid #e5e7eb;border-radius:8px;overflow:hidden"><tr style="background:#1a2332"><td colspan="2" style="padding:10px 14px;color:#4db6ac;font-weight:700;font-size:12px;text-transform:uppercase;letter-spacing:.8px">Wire Transfer — Wells Fargo</td></tr><tr style="background:#fff"><td style="padding:9px 14px;color:#9ca3af;width:45%">Bank Name</td><td style="padding:9px 14px;font-weight:600;color:#111827">Wells Fargo</td></tr><tr style="background:#f8fafc"><td style="padding:9px 14px;color:#9ca3af">Account Name</td><td style="padding:9px 14px;font-weight:600;color:#111827">Amansala LLC</td></tr><tr style="background:#fff"><td style="padding:9px 14px;color:#9ca3af">Account Number</td><td style="padding:9px 14px;font-weight:600;color:#111827">1639211174</td></tr><tr style="background:#f8fafc"><td style="padding:9px 14px;color:#9ca3af">Routing (RTN/ABA)</td><td style="padding:9px 14px;font-weight:600;color:#111827">121000248</td></tr><tr style="background:#fff"><td style="padding:9px 14px;color:#9ca3af">SWIFT/BIC</td><td style="padding:9px 14px;font-weight:600;color:#111827">WFBIUS6S</td></tr><tr style="background:#f8fafc"><td style="padding:9px 14px;color:#9ca3af">Account Address</td><td style="padding:9px 14px;font-weight:600;color:#111827">22219 N 39th Street, Phoenix AZ 85050, USA</td></tr><tr style="background:#fff"><td style="padding:9px 14px;color:#9ca3af">Bank Address</td><td style="padding:9px 14px;font-weight:600;color:#111827">21040 N Tatum Blvd, Phoenix AZ 85050, United States</td></tr><tr style="background:#f8fafc"><td style="padding:9px 14px;color:#9ca3af">Reference</td><td style="padding:9px 14px;font-weight:700;color:#2d6a6a">${ref}<br><span style="font-weight:400;color:#6b7280;font-size:12px">Please include retreat leader name and arrival date</span></td></tr></table><p style="font-size:13px;color:#6b7280;background:#fef9c3;border:1px solid #fde68a;border-radius:8px;padding:12px 16px">⚠️ <strong>All payments must be made via bank wire or bank transfer. Credit cards are not accepted.</strong><br>Please email <a href="mailto:retreats@amansala.com" style="color:#2d6a6a">retreats@amansala.com</a> with your confirmation once payment is sent.</p><table style="width:100%;border-collapse:collapse;margin:20px 0;font-size:13px;border:1px solid #e5e7eb;border-radius:8px;overflow:hidden"><tr style="background:#fff"><td style="padding:10px 14px;color:#9ca3af;width:40%">Retreat</td><td style="padding:10px 14px;font-weight:600;color:#111827">${retreat}</td></tr><tr style="background:#f8fafc"><td style="padding:10px 14px;color:#9ca3af">Dates</td><td style="padding:10px 14px;font-weight:600;color:#111827">${fmtRange}</td></tr><tr style="background:#fff"><td style="padding:10px 14px;color:#9ca3af">Balance Due</td><td style="padding:10px 14px;font-weight:700;color:#dc2626">${fmt$(balance)}</td></tr><tr style="background:#f8fafc"><td style="padding:10px 14px;color:#9ca3af">Payment Due</td><td style="padding:10px 14px;font-weight:700;color:#dc2626">${dueDate}</td></tr></table><p>We can't wait to welcome you to Amansala! 🌴</p></td></tr><tr><td style="background:#f1f5f9;padding:16px 32px;text-align:center;font-size:12px;color:#9ca3af">Amansala Bikini Boot Camp &amp; Retreat Center &nbsp;·&nbsp; Tulum, Mexico</td></tr></table></td></tr></table></body></html>`;

  _fbPending={bkId:bk.id,to:bk.leaderEmail,subject:`Final Balance Due — ${retreat} · ${fmtRange}`,html:finalHtml,balance};
  document.getElementById('fbPreviewSub').textContent=`To: ${bk.leaderEmail} — Balance ${fmt$(balance)}`;
  document.getElementById('fbPreviewFrame').srcdoc=finalHtml;
  openModal('finalBalancePreviewModal');
}

async function _fbConfirmSend(){
  if(!_fbPending)return;
  const {bkId,to,subject,html,balance}=_fbPending;
  const bk=AppData.bookings.find(b=>b.id===bkId);
  if(!bk){closeModal('finalBalancePreviewModal');return;}
  try{
    await _sendEmail(to.split(',').map(e=>e.trim()).filter(Boolean),subject,html,'retreats@amansala.com',_ccList(bk));
  }catch(e){showToast('Email failed: '+e.message);return;}
  closeModal('finalBalancePreviewModal');
  if(!bk.packageCustomPrices)bk.packageCustomPrices={};
...
// ── Send Missing Flight Info — nudges the retreat leader to chase down
// guests who haven't filled out the transport form yet. getTransportRoster()
// (modules/transport.js) is the same source of truth the Transport tab's
// Status view uses for "Missing" vs "Received", so this list always matches
// what staff already see there. Same live-preview-before-send pattern as
// Send Final Balance above.
let _mtPending=null; // {bkId, to, subject, html}
function sendMissingFlightInfoEmail(){
  if(!regSelBk)return;
  const bk=regSelBk;
  if(!bk.leaderEmail){showToast('No email on file for this retreat leader');return;}
  const{missing}=getTransportRoster(bk.id);
  if(!missing.length){showToast('Everyone has already submitted their transport info ✓');return;}
  const name=bk.leaderName||'Retreat Leader';
  const retreat=bk.retreatName||bk.leaderName||'Retreat';
  const listHtml=missing.map(g=>`<li style="margin-bottom:6px"><strong>${escHtml(g.name)}</strong></li>`).join('');
  const formLink=`${location.origin}/transport-form.html?bk=${bk.id}`;
  const mtHtml=`<!DOCTYPE html><html><body style="margin:0;padding:0;background:#f8fafc;font-family:Arial,Helvetica,sans-serif"><table width="100%" cellpadding="0" cellspacing="0"><tr><td><table style="max-width:580px;margin:20px auto;border-radius:12px;overflow:hidden;box-shadow:0 4px 20px rgba(0,0,0,.08)" cellpadding="0" cellspacing="0" width="100%"><tr><td style="background:#1a2332;padding:22px 32px"><span style="color:#4db6ac;font-size:22px;font-weight:700;letter-spacing:3px">AMANSALA</span></td></tr><tr><td style="background:#ffffff;padding:32px;color:#374151;font-size:14px;line-height:1.7"><p>Dear ${escHtml(name)},</p><p>We're writing to ask you to take a moment to review your group's transportation for <strong>${escHtml(retreat)}</strong>. We see that a few guests still haven't filled out the transport form.</p><p>Here's your group's transportation form link — just copy and send it to your guests. Once they fill it out, their information will appear under <strong>My Transportation</strong>.</p><table style="width:100%;border-collapse:collapse;margin:16px 0;font-size:13px;border:1px solid #e5e7eb;border-radius:8px;overflow:hidden"><tr style="background:#fff"><td style="padding:14px;text-align:center;word-break:break-all"><a href="${formLink}" style="color:#0e9494;font-weight:600">${formLink}</a></td></tr></table><table style="width:100%;border-collapse:collapse;margin:20px 0;font-size:13px;border:1px solid #e5e7eb;border-radius:8px;overflow:hidden"><tr style="background:#1a2332"><td style="padding:10px 14px;color:#4db6ac;font-weight:700;font-size:12px;text-transform:uppercase;letter-spacing:.8px">Still Missing</td></tr><tr style="background:#fff"><td style="padding:14px"><ul style="margin:0;padding-left:18px;color:#111827">${listHtml}</ul></td></tr></table><table width="100%" cellpadding="0" cellspacing="0"><tr><td align="center" style="padding:8px 0 4px"><a href="${teacherPortalLink(bk.id)}" style="background:#0e9494;color:#fff;padding:14px 32px;border-radius:8px;text-decoration:none;font-weight:700;font-size:15px;display:inline-block">Go to My Transportation →</a></td></tr></table><p style="margin-top:24px">Warm regards,<br><strong>Amansala Team</strong></p></td></tr><tr><td style="background:#f1f5f9;padding:16px 32px;text-align:center;font-size:12px;color:#9ca3af">Amansala Bikini Boot Camp &amp; Retreat Center &nbsp;·&nbsp; Tulum, Mexico</td></tr></table></td></tr></table></body></html>`;

  _mtPending={bkId:bk.id,to:bk.leaderEmail,subject:`Missing Transportation Info — ${retreat}`,html:mtHtml};
  document.getElementById('mtPreviewSub').textContent=`To: ${bk.leaderEmail} — ${missing.length} guest${missing.length!==1?'s':''} missing`;
  document.getElementById('mtPreviewFrame').srcdoc=mtHtml;
  openModal('missingTransportPreviewModal');
}

async function _mtConfirmSend(){
  if(!_mtPending)return;
  const{bkId,to,subject,html}=_mtPending;
  const bk=AppData.bookings.find(b=>b.id===bkId);
  if(!bk){closeModal('missingTransportPreviewModal');return;}
  try{
    await _sendEmail(to.split(',').map(e=>e.trim()).filter(Boolean),subject,html,'retreats@amansala.com',_ccList(bk));
  }catch(e){showToast('Email failed: '+e.message);return;}
  closeModal('missingTransportPreviewModal');
  showToast('Missing flight info request sent to '+to+' ✓');
  logActivity('email_sent',`Missing flight info request → ${bk.leaderName} (${to})`,bk.id);
  _mtPending=null;
}
