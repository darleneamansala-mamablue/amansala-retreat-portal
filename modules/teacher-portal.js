...
async function sendFinalBalanceEmail(){
  ...
  if(!confirm(`Send final balance request to ${bk.leaderEmail}?\n\nThis will send payment details for the remaining balance of ${fmt$(balance)}.`))return;
  const dueDate=(()=>{const d=new Date();d.setDate(d.getDate()+7);return d.toLocaleDateString('en-US',{month:'long',day:'numeric',year:'numeric'});})();
  const ref=`${bk.leaderName||'Retreat'} - ${bk.startDate?fmtDate(bk.startDate):''}`;
  const fmtRange=`${fmtDate(bk.startDate)} – ${fmtDate(bk.endDate)}`;
  const retreat=bk.retreatName||bk.leaderName||'Retreat';
  ...
  try{
    await _sendEmail(bk.leaderEmail.split(',').map(e=>e.trim()).filter(Boolean),`Final Balance Due — ${retreat} · ${fmtRange}`,finalHtml,'retreats@amansala.com');
  }catch(e){showToast('Email failed: '+e.message);return;}
  ...
  showToast('Final balance request sent to '+bk.leaderEmail+' ✓');
  logActivity('email_sent',`Final balance request → ${bk.leaderName} (${bk.leaderEmail}) — ${fmt$(balance)}`,bk.id);
}
