...
function renderEstQuote(lblId,bodyId){
  ...
  _sortedBlocked.forEach(room=>{
    ...
    const _tipNightsSum=sumGuestTipNights(reg,regSelBk,_eTipNights,gc);
  ...
  addHtml+=`<div class="eq-card" style="border-color:#fcd34d;background:#fffbf0">
    ...
    ${(totalCancellationAdj!==0||totalCreditAdj!==0||eqDiscountAmt>0)?`<div class="eq-row" style="font-weight:700;border-top:1px dashed #fcd34d;margin-top:4px;padding-top:6px"><span>Subtotal</span><span>${fmt$(+(totalRoomBase+totalPkg+totalTax+totalTip+totalCao).toFixed(2))}</span></div>`:''}
